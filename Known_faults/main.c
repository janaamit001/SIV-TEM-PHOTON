...
int generate_test_vectors()
{
	...
	unsigned char diff, diff1, diff2, diff3, diff4;
	...
	count = 0;
	diff = diff1 = diff2 = diff3 = diff4 = 0;
		...
		for( i1 = 1; i1 < 16; ++i1 ) {
			...
			if ( faulty_aead_decrypt(msg2, &mlen2, NULL, ct1, clen, ad, adlen, nonce, key, diff, pos ) == 0 ) {
				...
				if(count == 2) {
					...
					diff3 = rand() & 0xf;
					while(( diff3 == diff1 ) || ( diff3 == diff2 ) || (diff2 == 0) )
						diff3 = rand() & 0xf;
					diff = diff3;
				}
				...
				if(count == 3) {
					...
					diff4 = rand() & 0xf;
					while(( diff4 == diff1 ) || ( diff4 == diff2 ) || ( diff4 == diff3 ) || (diff2 == 0) )
						diff4 = rand() & 0xf;
					diff = diff4;
				}
