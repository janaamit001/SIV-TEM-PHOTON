...
void faulty_AddConstant(byte state[D][D], int round, const unsigned char diff, const unsigned char pos)
{
	unsigned int i, j;
	for(i = 0; i < D; i++)
		state[i][0] = state[i][0] ^ IC[i] ^ RC[round];
	...
	//copy states before S-box
	for(i = 0; i < D; i++)
		for(j = 0; j <  D; j++)
			st[i][j] = state[i][j];	
	
	//printf("faulty value is diff = %02x\n\n", diff);
	state[ pos / 8 ][ pos % 8 ] ^= diff;
...
//if(DEBUG) printf("--- Round %d ---\n", i);
		AddDomain(state, domain); //printf("after addDomain::\n"); PrintState(state);
		AddConstant(state, i); //printf("after addConstant::\n"); PrintState(state);
#ifndef _TABLE_
		//printf("...after S-box::\n");
		SubCell(state); 
		//PrintState(state);
		//printf("...after Siftrow::\n");
...
void faulty_Step(byte state[D][D], byte tweak_state[D/2][D], int start_rnd, unsigned char domain, const unsigned char diff, const unsigned char pos)
{
	int i;
	AddKey(state); //PrintState(state);
	AddTweak(state, tweak_state); //PrintState(state);
	for(i = start_rnd; i < start_rnd + STEP_INTER_ROUND_N-1; i++) {
//if(DEBUG) printf("--- Round %d ---\n", i);
		AddDomain(state, domain); 
		//printf("after addDomain::\n"); 
		//PrintState(state);
		AddConstant(state, i); 
	...
	//if(DEBUG) printf("--- Round %d ---\n", i);
		...
		//printf("after addDomain::\n"); 
		//PrintState(state);
		faulty_AddConstant(state, i, diff, pos); 
		//printf("after addConstant::\n"); 
		//PrintState(state);
#ifndef _TABLE_
		//printf("...after S-box::\n");
...
void faulty_TEM_PHOTON_Permutation(
	unsigned char *state_inout,
	const unsigned char *tweak_in, unsigned char domain, const unsigned char diff, const unsigned char pos)
{
    byte state[D][D];
	byte tweak_state[D/2][D];
    ...
    for (stp = 0; stp < STEP_N-1; stp++)
	{
		Step(state, tweak_state, stp * STEP_INTER_ROUND_N, domain);
	}
	
	faulty_Step(state, tweak_state, stp * STEP_INTER_ROUND_N, domain, diff, pos);
	AddKey(state); //PrintState(state);
	AddTweak(state, tweak_state); //PrintState(state);
